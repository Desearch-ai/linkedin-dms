...
These values are extracted from LinkedIn frontend traffic and may change without warning. When they change, the legacy backend provider path can fail even though cookies are still valid. The Chrome extension Sync Now path captures the live messaging GraphQL query IDs and safe variable templates from browser traffic instead; if that captured contract is missing or stale, refresh LinkedIn Messaging in the signed-in browser before retrying Sync Now. Sync Now replays only variables captured in the live template: current no-count contracts using just `mailboxUrn` for conversations and `conversationUrn` for messages are valid, and operators should not force or synthesize `count` when LinkedIn omits it. The extension also preserves safe syntax wrappers around runtime dynamic values (for example quoted or wrapped `mailboxUrn` / `conversationUrn` values) without storing the captured URN values themselves. LinkedIn reads now execute from an open LinkedIn page context and replay only allowlisted safe header values such as `x-li-lang` / `x-li-page-instance`; for messaging GraphQL contracts captured as POST-with-URL-query, the replay also preserves the safe request-shape pair LinkedIn used in Attempt #9 (`content-type: application/x-www-form-urlencoded` and `x-http-method-override`) plus the captured GET/POST method. Cookie, CSRF values, auth, profile, thread, and message values remain redacted; cookie/authorization headers are never replayed. Attempt #11 exposed a first-page replay class where the captured conversations contract included `query,count,mailboxUrn,nextCursor`; initial Sync Now now keeps the safe `query` shape, stores/renders `count` as first-page page-size metadata instead of preserving captured page counts, injects the runtime `mailboxUrn`, and omits cursor-like variables such as `nextCursor` unless an explicit runtime cursor is available. Message contracts with `deliveredAt,conversationUrn,countBefore,countAfter` are rendered with a fresh runtime timestamp, runtime conversation URN, first-page `countBefore`, and zero lookahead `countAfter` instead of replaying captured window values. If LinkedIn rejects a replay with HTTP 400, the popup error includes redacted request-shape diagnostics — queryId, variable keys/order, template sources including optional cursor entries, sanitized rendered variables, omitted optional variables, fetch context, replay method, URL path, query/body mode, browser/replay header names, and a short redacted response snippet — instead of only the generic status code.
